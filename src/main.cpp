...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < N; i++) {
		indices_vec = findIndices(cloud_q, inner_radius, outer_radius,
					  nodes_idx, self_distances);
		std::cout << "Found " << indices_vec.size() << " in rep " << i
			  << std::endl;
		...
		int closestFeature_idx = findClosestFeature(
		    features_vec, features_q[nodes_idx.back()]);
	...
	std::sort(
	    ...
	    [](const std::tuple<int, float> &t1,
	       const std::tuple<int, float> &t2) {
		    return std::get<1>(t1) <
			   std::get<1>(t2);  // or use a custom compare function
	    });
	// auto min_dist_point = std::min_element(target_distances.begin(),
	// target_distances.end());
	// auto min_dist_point = std::min_element(target_distances.begin(),
	// target_distances.end());

	// std::vector<int> min_point{min_dist_point -
	// target_distances.begin()};
